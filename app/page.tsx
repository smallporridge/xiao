...
{year:"2024 · 从第一针开始",color:"#efb9c5",left:{eyebrow:"CHAPTER 01 · 起针",title:"我先织点真的能穿出去的",aside:"帽子和背心，是我的毛线新手村装备。",layout:"duo",items:[
...
{year:"2025 · 配色会说话",color:"#d9c4e5",left:{eyebrow:"CHAPTER 07 · 披肩实验室",title:"反差越大胆，作品越有我的脾气",aside:"绿蓝、红边、焦糖橘和深咖，都坐到同一张桌上。",layout:"duo",items:[
...
]},right:{eyebrow:"CHAPTER 08 · 花样路线图",title:"我不只会织，也开始知道它为什么好看",aside:"一排踏脚石走过去，暗色里就开出了花。",layout:"duo",items:[
...
]},right:{eyebrow:"CHAPTER 10 · 毛茸茸警报",title:"成熟和幼稚，我当然都要",aside:"粉色先赢一半，灰色开衫负责稳住。",layout:"duo",items:[
...
]},right:{eyebrow:"CHAPTER 12 · 最新一页",title:"小繁花，先借小鸭戴一下",aside:"我喜欢的东西终于碰面，作品也立刻有了表情。",layout:"hero",items:[
work(2,"小繁花三角巾","2026 年","三角巾","我没有把它规规矩矩铺平，而是先借小鸭戴一下。作品一遇到角色，立刻就有了自己的表情。","かわいい")
]}},
{year:"未完待续 · 2024—2026",color:"#f1c0cb",left:{eyebrow:"TO BE CONTINUED · 13",title:"原来，我已经织了这么远",aside:"23件作品不是清单，是我一次次起针、拆掉、重来，也一直没弄丢可爱的证据。",layout:"duo",kind:"recap",items:[]},right:{eyebrow:"这一页，由朋友来写 · 14",title:"给未来还会继续织东西的你",aside:"",layout:"hero",kind:"letter",items:[]}}
];
...
const applyLeaf=(rawProgress:number,duration=0)=>{const active=turnRef.current,leaf=leafRef.current;if(!active||!leaf)return;const progress=Math.max(0,Math.min(.9995,rawProgress)),curve=Math.sin(progress*Math.PI),direction=active.dir==="next"?-1:1;const angle=isMobile&&active.dir==="prev"?-(1-progress)*179:direction*progress*179;const bend=direction*curve*(isMobile?.34:.62),lift=-curve*(isMobile?.6:1.4),scale=1-curve*(isMobile?.012:.018);leaf.style.transitionDuration=duration+"ms";const outerRadius=4+curve*(isMobile?22:31);leaf.style.borderRadius=isMobile?`3px ${outerRadius}px ${outerRadius+7}px 3px`:active.dir==="next"?`2px ${outerRadius}px ${outerRadius+8}px 2px`:`${outerRadius}px 2px 2px ${outerRadius+8}px`;leaf.style.setProperty("--curl",curve.toFixed(4));leaf.style.setProperty("--turn-progress",progress.toFixed(4));leaf.style.setProperty("--shade-opacity",(.08+curve*.7).toFixed(3));leaf.style.setProperty("--curl-width",(isMobile?11+curve*16:10+curve*15).toFixed(2)+"%");leaf.style.setProperty("--curl-opacity",(.2+curve*.66).toFixed(3));leaf.style.transform=`perspective(2100px) translateY(${lift}px) rotateY(${angle}deg) rotateZ(${bend}deg) scaleX(${scale})`;leaf.style.boxShadow=`${direction*curve*(isMobile?14:25)}px ${5+curve*11}px ${15+curve*31}px rgba(63,42,47,${.1+curve*.2})`};
const settleTurn=(mode:"complete"|"cancel")=>{if(!turnRef.current)return;const destination=mode==="complete"?1:0,remaining=Math.abs(destination-pendingDrag.current),speed=Math.abs(velocity.current);const duration=Math.round(Math.max(180,Math.min(470,190+remaining*310-speed*55)));setSettleMode(mode);requestAnimationFrame(()=>requestAnimationFrame(()=>applyLeaf(destination,duration)))};
